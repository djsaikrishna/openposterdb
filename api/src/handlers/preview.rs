...
pub async fn preview_poster(
    ...
    let mut preview_settings = preview_render_settings(cache::ImageType::Poster, badge_style, label_style, badge_size, position, badge_direction, ratings_limit, ratings_order, ratings_exclude);
    ...
    let buf = tokio::task::spawn_blocking(move || {
        generate::render_poster_sync(poster_png, &badges, &font, quality, position, badge_style, label_style, badge_direction, target_width, badge_scale, badge_size, split)
    })
    .await
...
mod tests {
    ...
    fn sample_poster_renders_with_badges() {
        ...
        let badges = sample_badges();
        let result = generate::render_poster_sync(&SAMPLE_POSTER_PNG, &badges, &font, 85, BadgePosition::BottomCenter, BadgeStyle::Horizontal, LabelStyle::Text, BadgeDirection::Horizontal, 500, 1.0, BadgeSize::Medium, false);
    ...
    fn sample_poster_renders_with_no_badges() {
        let font = ab_glyph::FontArc::try_from_slice(crate::FONT_BYTES).unwrap();
        let result = generate::render_poster_sync(&SAMPLE_POSTER_PNG, &[], &font, 85, BadgePosition::BottomCenter, BadgeStyle::Horizontal, LabelStyle::Text, BadgeDirection::Horizontal, 500, 1.0, BadgeSize::Medium, false);
        let buf = result.expect("rendering should succeed");
        assert_eq!(buf[0], 0xFF);
        assert_eq!(buf[1], 0xD8);
    }
