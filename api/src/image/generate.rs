...
pub struct ImageParams<'a> {
    ...
    /// Non-rating overlay badges (quality tiers, language flag/code) appended
    /// after the rating badges.
    pub overlay_badges: &'a [badge::OverlayBadge],
...
pub async fn generate_poster(params: ImageParams<'_>) -> Result<Vec<u8>, AppError> {
    ...
    let overlay_badges = overlay_badges.to_vec();
    let font = font.clone();
    let buf = tokio::task::spawn_blocking(move || {
        render_poster_sync(&poster_bytes, &badges, &overlay_badges, &font, quality, poster_position, badge_style, label_style, badge_appearance, badge_direction, target_width, badge_scale, badge_size, poster_badge_split, poster_fit)
    })
    .await
...
/// to match the rating badges' orientation, ready to append to the badge list.
...
pub fn render_poster_sync(
    ...
    if !badges.is_empty() || !overlay_badges.is_empty() {
        let mut badge_images: Vec<RgbaImage> = if badge_style.is_vertical() {
        ...
        // Quality / language overlay badges are appended after the ratings.
        badge_images.extend(render_overlay_images(overlay_badges, font, badge_style.is_vertical(), badge_appearance, badge_scale));
...
pub async fn generate_logo(
    ...
    overlay_badges: Vec<badge::OverlayBadge>,
    font: FontArc,
    badge_style: BadgeStyle,
    label_style: LabelStyle,
    badge_appearance: BadgeAppearance,
    ...
    let buf = tokio::task::spawn_blocking(move || render_logo_sync(&logo_bytes, &badges, &overlay_badges, &font, badge_style, label_style, badge_appearance, target_width, badge_scale))
        .await
        .map_err(|e| AppError::Other(e.to_string()))??;
...
pub async fn generate_backdrop(
    ...
    let buf = tokio::task::spawn_blocking(move || render_backdrop_sync(&backdrop_bytes, &badges, &overlay_badges, &font, quality, position, badge_style, label_style, badge_appearance, badge_direction, target_width, badge_scale, badge_size, edge_inset_x, edge_inset_y))
        .await
        .map_err(|e| AppError::Other(e.to_string()))??;
...
mod tests {
    ...
    async fn generate_logo_acquires_and_releases_semaphore() {
        ...
        let result = generate_logo(png, vec![], vec![], font, BadgeStyle::Horizontal, LabelStyle::Text, BadgeAppearance::default(), sem.clone(), 500, 1.0).await;
        assert!(result.is_ok());
    ...
    async fn generate_logo_closed_semaphore_returns_error() {
        ...
        let result = generate_logo(png, vec![], vec![], font, BadgeStyle::Horizontal, LabelStyle::Text, BadgeAppearance::default(), sem, 500, 1.0).await;
        assert!(result.is_err());
    }
    ...
    async fn generate_backdrop_acquires_and_releases_semaphore() {
        ...
        let result = generate_backdrop(png, vec![], vec![], font, 85, BadgePosition::TopRight, BadgeStyle::Horizontal, LabelStyle::Text, BadgeAppearance::default(), BadgeDirection::Vertical, sem.clone(), 1280, 1.0, BadgeSize::Medium, 0, 0).await;
        assert!(result.is_ok());
        assert_eq!(sem.available_permits(), 1);
    }
    ...
    async fn generate_backdrop_closed_semaphore_returns_error() {
        ...
        let result = generate_backdrop(png, vec![], vec![], font, 85, BadgePosition::TopRight, BadgeStyle::Horizontal, LabelStyle::Text, BadgeAppearance::default(), BadgeDirection::Vertical, sem, 1280, 1.0, BadgeSize::Medium, 0, 0).await;
        assert!(result.is_err());
    }
    ...
    async fn semaphore_bounds_concurrent_logo_renders() {
        ...
        let handle = tokio::spawn(async move {
            generate_logo(png, vec![], vec![], font2, BadgeStyle::Horizontal, LabelStyle::Text, BadgeAppearance::default(), sem2, 500, 1.0).await
        });
