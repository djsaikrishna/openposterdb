...
- **Clear everything** — the **Clear cache** button on the dashboard wipes all rendered images and raw downloads on disk, every `image_meta` / `available_ratings` row, and every in-memory image cache (including the upstream TMDB/Fanart.tv image-list and ratings caches). Images regenerate from scratch on the next request, so the first load of each title afterwards is slower. This is the path that guarantees a fully clean re-fetch.
